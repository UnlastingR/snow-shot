...
pub enum ElementError {
    ...
    Automation(#[from] uiautomation::errors::Error),
    #[error("window is blacklisted")]
    Blacklisted,
}

type UIAutomationError = ElementError;
...
impl UIElements {
    ...
    pub fn init_cache(&mut self) -> Result<(), UIAutomationError> {
        ...
        // SAFETY: GetSystemMetrics reads the current virtual-desktop geometry.
        ...
        let root_element_rect = uiautomation::types::Rect::new(
            virtual_left,
            virtual_top,
            virtual_left.saturating_add(virtual_width),
            virtual_top.saturating_add(virtual_height),
        );
    ...
    fn get_element_from_cache(
        ...
        let rect = max_level_rect?;
        let element_rtree_rect =
