...
fn overlay_image(
    ...
    let target_width = target_image.width() as usize;
    let target_height = target_image.height() as usize;
    let row_bytes = target_width * channel_count;
    let target_pixels = target_image.as_bytes();
    for y in 0..target_height {
        let source_start = y * row_bytes;
        let target_start = ((offset_y + y) * image_width + offset_x) * channel_count;
        image_pixels[target_start..target_start + row_bytes]
            .copy_from_slice(&target_pixels[source_start..source_start + row_bytes]);
    }
}
...
impl ScrollIndex {
    pub fn new(dimension: usize) -> Self {
        let index_params = HNSWParams::<f32> {
            ef_search: 24,
            ef_build: 12,
            ..Default::default()
        };
...
impl ScrollScreenshotService {
    ...
    pub fn init(&mut self, config: ScrollScreenshotConfig) {
        self.top_image_list.clear();
        self.bottom_image_list.clear();
        self.current_direction = config.direction;
        self.image_width = 0;
        self.image_height = 0;
        self.top_image_size = 0;
        self.bottom_image_size = 0;
        self.corner_threshold = config.corner_threshold;
        self.descriptor_patch_size = config.descriptor_patch_size;
        self.min_size_delta = config.min_size_delta;
        ...
        self.try_rollback = config.try_rollback;
    ...
    fn get_crop_region(&self, delta_size: i32) -> CropRegion {
        ...
        if self.current_direction == ScrollDirection::Vertical {
            let start_position = image_height - delta_size.unsigned_abs();
            if delta_size > 0 {
            ...
            let start_position = image_width - delta_size.unsigned_abs();
            if start_position > 0 {
                region = CropRegion::new(
                    start_position,
                    0,
    ...
    pub fn get_offsets<'a>(
        ...
        for (count, scroll_index, origin_idx, new_idx) in offset_counts.values() {
            if *count > max_count {
...
mod tests {
    ...
    #[test]
    fn stitches_overlapping_vertical_frames() {
        let source = patterned_image(96, 144);
        let first = image::imageops::crop_imm(&source, 0, 0, 96, 96).to_image();
        let second = image::imageops::crop_imm(&source, 0, 24, 96, 96).to_image();
        ...
        assert!(
            service
                .handle_image(DynamicImage::ImageRgba8(first), ScrollImageList::Bottom)
                .0
                .is_some()
        );
        assert!(
            service
                .handle_image(DynamicImage::ImageRgba8(second), ScrollImageList::Bottom)
                .0
                .is_some()
        );

        let stitched = service
            .export()
            .expect("two frames should export")
            .to_rgba8();
        assert_eq!(stitched.dimensions(), (96, 120));
        assert_eq!(
            stitched,
            image::imageops::crop_imm(&source, 0, 0, 96, 120).to_image()
        );
    }
}
