...
- 浮动工具栏支持复制、PNG 原生保存、多窗口置顶贴图和本地 OCR。
- OCR 沿用 `OcrService + paddle-ocr-rs + ONNX Runtime + PP-OCRv4`；首次使用自动安装官方模型，结果直接覆盖在当前选区内，不创建独立结果窗口。
- OCR 文本会合并同一段落内的视觉换行，并保留句末、列表与段落边界。
- 选区工具栏已接入矩形、椭圆、画笔、高亮、线段、箭头、序号、马赛克、模糊、橡皮擦与取色，标注历史支持撤销和重做。
- 标注后的统一工作图用于复制、保存和贴图，避免不同输出路径丢失编辑结果。
- 选区完成后支持 `Ctrl+C` 复制、`Ctrl+S` 保存、`Ctrl+A` 以 S 曲线动画扩展到全屏。
...
- `src/ocr_workflow.rs`：按需模型安装、OCR worker 和会话空闲释放
- `../../crates/snowshot-annotate`：纯 Rust 标注图像文档、像素工具与撤销/重做
...
1. 补齐文字标注、线宽和颜色预设。
2. 将当前 CPU 标注工作图升级为 GPU 画布，同时保持文档模型不变。
3. 接入配置持久化与开机启动。
