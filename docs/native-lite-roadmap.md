...
- 选区支持框内移动与上下左右/四角缩放，Shift 保持比例，Ctrl 保持中心；贴图支持不可见四角命中区等比缩放和 Ctrl+滚轮缩放。
- 选区与贴图的等比缩放使用连续对角线投影，避免横轴/纵轴控制切换造成手感突变。
- 已接入 `Alt+F11` 全局隐藏/显示贴图；每张贴图可独立关闭，并使用右侧和下侧 6px 高斯模糊、完整衰减且点击穿透的阴影。
- Windows 贴图已从 Slint 独立窗口迁移到 Win32 + DirectComposition；截图 Surface 在创建时上传一次，缩放不再重建 Slint/FemtoVG 绘制表面。
- Windows Native App 已接入现有本地 OCR，模型按需安装，结果直接覆盖当前选区并支持文本复制。
- 已新增 `crates/snowshot-annotate`，承载矩形、椭圆、画笔、高亮、线段、箭头、序号、马赛克、模糊、橡皮擦、取色及撤销/重做。
- `Ctrl+C`、`Ctrl+S` 与带动画的 `Ctrl+A` 已接入选区工作流。
- 贴图最小保持初始显示尺寸的 30%，且不低于 96×64 物理像素，阴影模糊、偏移与窗口留白随内容同比缩放。
- 区域截图不会隐藏设置窗口；冻结帧在覆盖层显示前完成选区重置，避免主窗切换和旧帧造成的闪烁。
- 截图覆盖层已改为不透明全屏窗口加四块外围蒙层，拖动时实时显示未变暗选区，并将高频指针移动合并到 8ms 一次；Esc 增加截图会话级按键状态兜底。
- Windows 下一步补齐文字标注、线宽与颜色预设，再推进 GPU 标注画布。
